...
<script setup>
    ...
    function addNewItem() {
        aArray.value.push({
            ...
            sentence: newSentence.value, dealt: false
        })
    }
...
<template>
    <ChildComponent castVar /><!-- String -->
    <p :class="applyClass">{{ aVariable }}</p>
    <p>{{ aArray[0].sentence }}</p>
    <p>{{ arrayItem }}</p>
    <button class="w-fit border-4 border-spacing-0 border-green-500 p-1" @click="unneededFunc('down')">One Step
        Backward</button><br />
    <button class="w-fit border border-spacing-1 border-green-500 p-2" @click="unneededFunc('up')">One Topic
        Forward</button>
    <input typw="text" class="border border-rose-500" v-model="newSentence" placeholder="type here then">
    <button class="w-fit border border-spacing-1 border-green-500 p-2" @click="addNewItem()">One New Sentence</button>
    <p class="text-3xl font-light my-5" :style="applyStyle">{{ aComputedVariable }}</p>
    <br />
    <ul>
        <p class="border-2 border-yellow-300">sEnTeNcEd: one two three ...</p>
