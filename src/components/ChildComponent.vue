...
<script setup>
   ...
   class wEnabled extends wContainer {
      ...
      wEnabledT(param) {
         return param == 'test' ? 'testFlight' : 'testFlight'
      }
   }
...
<template>
   <div class="child">
      <h2 class="bg-blend-color-dodge">{{ castVar }}:
         then {{ aa.wEnabled() }} features of this tag
         and {{ aa.wEnabledT('testD') }} features of again this tag
      </h2>
   </div>
</template>
